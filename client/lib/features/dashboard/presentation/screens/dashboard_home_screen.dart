...
//                     iconBg: const Color(0xFFBA7517).withValues(alpha: 0.08),
...
// ── Semantic accent colors used across the dashboard ─────────────────────────
// Defined once so dark/light variants stay consistent.
const Color _kAccentGreen      = Color(0xFF1D9E75); // Total / Orders
const Color _kAccentDeepGreen  = Color(0xFF0F6E56); // Delivered
const Color _kAccentGold       = Color(0xFFBA7517); // Delivery staff / warning
const Color _kAccentBlue       = Color(0xFF2563EB); // Out-for-delivery
...
class _DashboardHomeScreenState extends State<DashboardHomeScreen> {
  ...
  // ── Filter pill ───────────────────────────────────────────────────────────
  // Uses colorScheme so it adapts to dark mode automatically.
  Widget _filterPill({
    required String label,
    required VoidCallback? onTap,
  }) {
    ...
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 9),
        decoration: BoxDecoration(
          // surface adapts: white in light, dark card color in dark
          color: cs.surface,
          borderRadius: BorderRadius.circular(18),
        ...
        child: Row(
          ...
          children: [
            Flexible(
              child: Text(
                ...
                style: TextStyle(
                  fontSize: 12,
                  fontWeight: FontWeight.w700,
                  // onSurface adapts to dark text automatically
                  color: cs.onSurface,
                ),
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
            ...
            Icon(
              Icons.keyboard_arrow_down_rounded,
              size: 18,
              color: onTap == null ? cs.onSurfaceVariant : cs.primary,
            ),
          ],
        ),
      ),
  ...
  Future<void> _openMealFilterSheet() async {
    ...
    await showModalBottomSheet<void>(
      ...
      backgroundColor: Colors.transparent,
      builder: (ctx) {
        // Read theme inside builder so it uses the sheet's context.
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                const SizedBox(width: 12),
                // Date pill — primary tint, works on both themes
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
                  decoration: BoxDecoration(
            ...
            // ── Today's orders (horizontal strip) ────────────
            Row(
              children: [
                const Expanded(
                  child: _SectionLabel(label: "Today's orders"),
            ...
            Container(
              decoration: BoxDecoration(
                // cardBackground equivalent: cs.surfaceContainerLow adapts
                color: cs.surfaceContainerLow,
                borderRadius: BorderRadius.circular(18),
                border: Border.all(color: cs.outline),
                boxShadow: [
                  BoxShadow(
                    // theme-aware shadow: uses colorScheme.shadow
                ...
                children: [
                  ...
                  Expanded(
                    child: _StatCard(
                      ...
                      iconBg: _kAccentGreen.withValues(alpha: 0.12),
                      iconColor: _kAccentGreen,
                      valueColor: _kAccentGreen,
                    ),
                  ),
                  const SizedBox(width: 10),
                  Expanded(
                    child: _StatCard(
                      label: 'Delivery',
                      value: _deliveryStaffCount.toString(),
                      icon: Icons.delivery_dining_outlined,
                      iconBg: _kAccentGold.withValues(alpha: 0.12),
                      iconColor: _kAccentGold,
                      valueColor: _kAccentGold,
                    ),
                  ),
                ],
              ),
  ...
  Widget build(BuildContext context) {
    return Text(
      ...
      style: TextStyle(
        fontSize: 11,
        fontWeight: FontWeight.w600,
        // onSurfaceVariant gives muted label colour in both themes
...
class _OrderStatusCountTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: DecoratedBox(
        decoration: BoxDecoration(
          // surfaceContainer gives a slightly elevated card tone in dark mode
